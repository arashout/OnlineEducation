...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        pylab.figure(count)
        pylab.plot(x,y,"bo",label="Actual Temp")
        pylab.plot(x,y_pred,"-r", label="Model")
        pylab.ylabel("Temperatures [deg]")
        pylab.xlabel("Time [years]")
        pylab.title("Temp. as function of time - Fit Poly Deg = {0}".format(
            len(model) - 1))
        pylab.legend()
        pylab.text(1980,11,"$R^2$={0:2f}".format(r_squared(y, y_pred)))
        count += 1
    pylab.show()
