...
def isValidWord(word, hand, wordList):
    ...
    """
    freqWordDict = getFrequencyDict(word)
    copyHand = hand.copy()
    for letter in freqWordDict.keys():
        numLetters = copyHand.get(letter)
        if(numLetters is not None):
            numLeft = copyHand[letter] - freqWordDict[letter]
            if(numLeft < 0):
                return False
        else:
            return False
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while True:
        ...
        if(calculateHandlen(hand) == 0):
            print(
                "Run out of letters. Total score: {0} points.".format(totalScore))
            break
        # Display the hand
        ...
        # Otherwise (the input is not a single period):
            ...
            if(isValidWord(userWord, hand, wordList) is False):
